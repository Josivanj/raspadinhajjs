...
class Core
{
    ...
    public static function payBonusVip(Wallet $wallet, $price)
    {
        ...
        if ($setting->activate_vip_bonus) {
            try {
                ...
                // Atualiza o nível VIP baseado nos pontos atuais
                self::updateVipLevel($wallet);
     ...
     * @return void
     */
    public static function updateVipLevel(Wallet $wallet)
    {
        $setting = Setting::first();
        ...
        foreach ($vips as $vip) {
            ...
            if (!$checkVip) {
                ...
                // REMOVIDO: Bônus automático ao subir de nível
...
//        \Log::info('------------------------------------------------------------');
                        ...
                        if(empty($wallet->balance_deposit_rollover) || $wallet->balance_deposit_rollover <= 0) {
                            ...
                            if($wallet->balance_deposit_rollover >= $bet) {
                                ...
                                if($wallet->balance_deposit_rollover === 0) {
                                    ...
                                    // NUNCA volta para balance, sempre vai para balance_bonus quando tem rollover
                                    $wallet->increment('balance_bonus', $win);
                                }
                            }else{
    ...
    {
        return [
          'play_fiver' => 'Play Fiver', // Adicione essa linha
            'source' => 'Código Fonte',
            'drakon' => 'Drakon API',
...
private static function PayLoss($user, $affiliate, $bet, $changeBonus)
{
    ...
    if (!empty($affiliate)) {
        ...
        if (!empty($affHistoryRevshare) && $changeBonus === 'balance') {
...
private static function PayWin($user, $affiliate, $type, $amount, $changeBonus)
{
    ...
    if ($setting->revshare_reverse) {
        if ($type == 'win' && !empty($user->inviter) && $changeBonus === 'balance') {
            if (!empty($affiliate)) {
                $affHistoryRevshare = AffiliateHistory::where('user_id', $user->id)
                    ->where('commission_type', 'revshare')
                    // ->where('deposited', 1)
    ...
    public static function getCustom()
    {
        ...
        if(Cache::has('custom')) {
            ...
            $custom = CustomLayout::select(
                ...
                'feedback_page_form_background_color',
            )->first();
    ...
    public static function getCustomLayoutCss()
    {
        $custom = CustomLayout::first();

        return "
            :root {
                --ci-font-family: {$custom->font_family_default};
                ...
                --ci-gray-dark-color: {$custom->gray_dark_color};
                --ci-gray-light-color: {$custom->gray_light_color};
                --ci-gray-medium-color: {$custom->gray_medium_color};
                --ci-gray-over-color: {$custom->gray_over_color};
                --ci-title-color: {$custom->title_color};
                --ci-text-color: {$custom->text_color};
                --ci-sub-text-color: {$custom->sub_text_color};
                --ci-placeholder-color: {$custom->placeholder_color};
                --ci-background-color: {$custom->background_color};
                --ci-border-radius: {$custom->border_radius};

                --ci-background-base: {$custom->background_base};
                --ci-background-base-dark: {$custom->background_base_dark};

                --ci-input-primary: {$custom->input_primary};
                --ci-input-primary-dark: {$custom->input_primary_dark};

                --ci-carousel-banners: {$custom->carousel_banners};
                --ci-carousel-banners-dark: {$custom->carousel_banners_dark};

                --ci-sidebar-color: {$custom->sidebar_color};
                --ci-sidebar-color-dark: {$custom->sidebar_color_dark};

                --ci-navtop-color: {$custom->navtop_color};
                --ci-navtop-color-dark: {$custom->navtop_color_dark};

                --ci-side-menu: {$custom->side_menu};
                --ci-side-menu-dark: {$custom->side_menu_dark};

                --ci-footer-color: {$custom->footer_color};
                --ci-footer-color-dark: {$custom->footer_color_dark};

                --ci-card-color: {$custom->card_color};
                --ci-card-color-dark: {$custom->card_color_dark};

                --ci-ultimos-ganhos-player-color: {$custom->ultimos_ganhos_player_color};
                --ci-ultimos-ganhos-valor-color: {$custom->ultimos_ganhos_valor_color};

                /* Cores do formulário de feedback */
                --ci-feedback-page-background: {$custom->feedback_page_page_background};
                --ci-feedback-page-fade: {$custom->feedback_page_background};
                --ci-feedback-form-background: {$custom->feedback_page_form_background};
                --ci-feedback-title-color: {$custom->feedback_page_title_color};
                --ci-feedback-text-color: {$custom->feedback_page_text_color};
                --ci-feedback-share-title-color: {$custom->feedback_share_title_color};
                --ci-feedback-share-background: {$custom->feedback_share_background_color};
                --ci-feedback-form-text-color: {$custom->feedback_page_form_text_color};
                --ci-feedback-form-button-color: {$custom->feedback_page_form_button_color};
                --ci-feedback-form-button-text-color: {$custom->feedback_page_form_button_text_color};
                --ci-feedback-form-background-color: {$custom->feedback_page_form_background_color};
            }
        ";
    }
}
