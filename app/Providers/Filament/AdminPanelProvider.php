                    ...
                    auth()->user()->hasRole('admin') ?
                        ...
                        ->items([
                            NavigationItem::make('dashboard')
                                ...
                                ->isActiveWhen(fn () => request()->routeIs('filament.central.pages.dashboard-admin')),

                            NavigationItem::make('settings')
                                ->icon('heroicon-o-cog-6-tooth')
                                ->label(fn (): string => 'Configurações')
                                ->url(fn (): string => SettingResource::getUrl())
                                ->isActiveWhen(fn () => request()->routeIs('filament.central.resources.settings.*'))
                        ...
                        NavigationGroup::make('Pagamentos')
                            ->items([
                                NavigationItem::make('gateway')
                                    ...
                                    ->isActiveWhen(fn () => request()->routeIs('filament.central.pages.gateway-page'))
                                    ->visible(fn(): bool => auth()->user()->hasRole('admin')),
                            ])
                        : NavigationGroup::make()
                    ,
                        ...
                        NavigationGroup::make('Customização')
                            ->items([
                                ...
                                NavigationItem::make('custom-layout')
                                    ...
                                    ->isActiveWhen(fn () => request()->routeIs('filament.central.pages.layout-css-custom'))
                                    ->visible(fn(): bool => auth()->user()->hasRole('admin'))
                            ])
                        : NavigationGroup::make()
                    ,
                    auth()->user()->hasRole('admin') ?
                        NavigationGroup::make('Administração')
                            ->items([
                                ...
                                ->isActiveWhen(fn () => request()->routeIs('filament.central.resources.affiliate-withdraws.*'))
                                ->visible(fn(): bool => auth()->user()->hasRole('admin')),
                            ])
                        : NavigationGroup::make()
                    ,
